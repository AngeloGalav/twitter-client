...
exports.createWordCloud = (req, res, next) => {
    if (req.data.statuses.length > 0 && req.genStats) {
        ...
        for (let i = 0; i < req.data.statuses.length; i++) {
            //rimozione link
            let textNoUrl = req.data.statuses[i].full_text.replace(
                urlRegex,
                ""
            );
            ...
            //rimozione stopword
            let words = sw.removeStopwords(
                textNoNewLine.toLowerCase().split(" "),
                sw[`${req.data.statuses[i].lang}`] || sw.en
            );
