...
export const Tweet = ({ tweet, setCenter }) => {
    ...
    const parseText = (text) => {
        ...
        let textHashtagLink = textLink.replace(
            hashtagRegex,
            (hashtag) =>
                `<a class="text-primary hover:underline" href=http://localhost:3000/tweets/Hashtag?q=${hashtag.substring(
                    1
                )}>${hashtag}</a>`
        );
        ...
        return textHashtagLink.replace(
            userNameRegex,
            (userName) =>
                `<a class="text-primary hover:underline " href=http://localhost:3000/tweets/Username?q=${userName
                    .replace("@", "")
                    .replace(" ", "")}>${userName}</a>`
        );
    };
