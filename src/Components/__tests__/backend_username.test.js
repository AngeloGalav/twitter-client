...
describe('Fetch username', () => {
    it("should return error cuz no params", async () => {
        try {
            await axios.get("http://localhost:8000/api/Username", {
    ...
    it("should return error cuz no date", async () => {
        try {
            await axios.get("http://localhost:8000/api/Username?q=ciao&position=null&radius=null&tweetCount=15", {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(true).toBe(false)
        } catch (error) {
    ...
    it("should return error cuz no query", async () => {
        try {
            await axios.get(`http://localhost:8000/api/Username?${defaultParams}`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(true).toBe(false)
        } catch (error) {
    ...
    it("should return some tweets", async () => {
        try {
            const {data} = await axios.get(`http://localhost:8000/api/Username?q=ciao&${defaultParams}`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
    ...
    it("should generate statistics", async() => {
        try {
            const {data} = await axios.get(`http://localhost:8000/api/Username?q=ciao&${defaultParams}&genStats=true`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(data.statuses.length).toBeGreaterThanOrEqual(0);
            if(data.statuses.length > 0){
    ...
    it("should not generate statistics", async() => {
        try {
            const {data} = await axios.get(`http://localhost:8000/api/Username?q=ciao&${defaultParams}&genStats=false`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(data.statuses.length).toBeGreaterThanOrEqual(0);
