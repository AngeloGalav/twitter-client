...
describe('Fetch hashtag', () => {
    it("should return error cuz no params", async () => {
        try {
            await axios.get("http://localhost:8000/api/Hashtag", {
    ...
    it("should return error cuz no date", async () => {
        try {
            await axios.get("http://localhost:8000/api/Hashtag?q=ciao&position=null&radius=null&tweetCount=15", {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(true).toBe(false)
        } catch (error) {
    ...
    it("should return error cuz no query", async () => {
        try {
            await axios.get(`http://localhost:8000/api/Hashtag?${defaultParams}`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(true).toBe(false)
        } catch (error) {
    ...
    it("should return some tweets", async () => {
        try {
            const {data} = await axios.get(`http://localhost:8000/api/Hashtag?q=ciao&${defaultParams}`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
    ...
    it("should return only italians", async () => {
        try {
            const {data} = await axios.get(`http://:8000/api/Hashtag?q=ciao&${defaultParams}&onlyItalian=true`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(data.statuses.length).toBeGreaterThanOrEqual(0);
    ...
    it("should generate statistics", async() => {
        try {
            const {data} = await axios.get(`http://localhost:8000/api/Hashtag?q=ciao&${defaultParams}&genStats=true`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(data.statuses.length).toBeGreaterThanOrEqual(0);
            if(data.statuses.length > 0){
    ...
    it("should not generate statistics", async() => {
        try {
            const {data} = await axios.get(`http://localhost:8000/api/Hashtag?q=ciao&${defaultParams}&genStats=false`, {
                adapter: require('axios/lib/adapters/http'), //inserire sempre questo
            }) // no params
            expect(data.statuses.length).toBeGreaterThanOrEqual(0);
